# im-bridge-tenant-routing Specification

## Purpose
Define how IM Bridge loads tenant definitions, resolves the owning tenant for every inbound message, propagates tenant context through to outbound delivery and backend calls, and resolves tenant-scoped credentials — so a single bridge process can safely serve multiple AgentForge projects without cross-tenant leakage.
