# live-artifact-projection Specification

## Purpose
Define the backend projection pipeline that turns a live-artifact block's reference into a BlockNote JSON fragment: the `LiveArtifactProjector` interface, its registry, the batch REST endpoint, per-kind projector behavior for `agent_run`, `cost_summary`, `review`, and `task_group`, and schema-version evolution for `view_opts`.
