# im-bridge-command-plugin-registry Specification

## Purpose
Define the YAML-manifest plugin registry that the IM Bridge uses to load, invoke, scope, hot-reload, and accept marketplace-installed command plugins — covering manifest schema, the http/mcp/builtin invoke kinds, tenant scoping, filesystem hot-reload, and the marketplace distribution seam — so operators can extend the bridge's slash-command surface without recompiling the binary.
