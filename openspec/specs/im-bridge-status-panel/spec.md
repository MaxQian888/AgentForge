# im-bridge-status-panel Specification

## Purpose
Real-time IM connection health, message queue status, and retry controls for configured IM bridges (Feishu, WeCom, QQ, DingTalk, etc.). Operators monitor per-platform health, queue depth, rate limits, recent activity, and can retry failed deliveries or send test messages.
