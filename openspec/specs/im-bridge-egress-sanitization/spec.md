# im-bridge-egress-sanitization Specification

## Purpose
Define the outbound text sanitization contract for AgentForge IM Bridge, covering provider-aware policy modes (strict/permissive/off), broadcast-mention stripping, zero-width character removal, oversized text truncation or segmentation, and the requirement that every provider declares a non-zero TextLengthLimit.
## Requirements
