...
## Purpose
Define the canonical project-level role-based access control model for AgentForge so every project-scoped write action — whether invoked directly through the API, through the UI, or through an agent action initiated by a human — is authorized by the initiating human's project role against a single server-side action-to-role matrix. This specification covers role taxonomy, the central RBAC matrix, agent-action authorization, project ownership invariants, and the canonical permissions endpoint consumed by the frontend.
