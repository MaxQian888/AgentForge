# live-artifact-subscription Specification

## Purpose
Define the WebSocket subscription plane that keeps live-artifact blocks fresh: per-asset subscription filters derived from each block's projector, the `knowledge.asset.live_artifacts_changed` event, coalescing and per-block rate-capping, and the client-side partial re-projection flow.
