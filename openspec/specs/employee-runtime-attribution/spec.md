# employee-runtime-attribution Specification

## Purpose

Defines how workflow runs and their spawned agent runs attribute work to a Digital Employee. Covers run-level default attribution persisted on each workflow run, step-level override precedence, propagation of trigger-time acting-employee identifiers through the unified dispatch router, cross-project and archived-target rejection semantics, and observability of acting-employee attribution on run read APIs.

This capability exists so that consumers can query and reason about which Digital Employee a workflow run represents without having to join through every spawned agent run, while still allowing individual steps to override the default attribution when warranted.
