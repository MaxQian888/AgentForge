# project-audit-log Specification

## Purpose
Define the project-scoped audit log capability so AgentForge can record every gated write against project resources — including RBAC denials — in a dedicated, queryable, sanitized store whose failures never break the underlying business path, and surface those events to admins/owners through a guarded API and workspace view.
