...
The system SHALL represent marketplace installation and consumption state through a typed contract that covers item identity, item type, selected version, install status, warning or failure reason, consumer surface, and downstream record identity when available. The contract MUST support plugin, skill, and role marketplace items without collapsing them into one generic installed-items list.
