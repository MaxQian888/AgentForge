...
The system SHALL treat assignment of a task to an active agent member as a task-centered dispatch request instead of only updating assignee fields. When an authenticated caller assigns a task to an agent member, the system MUST persist the assignment, keep task workflow state aligned with the assignment, evaluate dispatch readiness for that task/member/project combination, and return a structured dispatch outcome for the same request.

#### Scenario: Assigning a task to an agent starts dispatch
- **WHEN** an authenticated caller assigns an existing task to an active agent member in the same project and the task has no active agent run
...
The system SHALL expose assignment and dispatch as a layered result, so API clients, WebSocket consumers, notifications, and IM command handlers can distinguish between assignment success and runtime startup success. Successful dispatch MUST continue to emit runtime lifecycle signals, while blocked or failed dispatch attempts MUST emit an explicit dispatch result that consumers can render without inferring from missing runtime events.
