...
#### Scenario: Unsupported QQ richer update falls back to text with explicit reason
- **WHEN** a QQ-targeted delivery requests a richer card, mutable update, or unsupported interaction path that the active QQ reply target cannot honor
- **THEN** the Bridge falls back to a QQ-supported text or follow-up delivery before sending the message
- **AND** the fallback metadata explains that the richer QQ update plan was not usable

#### Scenario: Unsupported QQ Bot richer update falls back to text with explicit reason
- **WHEN** a QQ Bot-targeted delivery requests a richer card, mutable update, or unsupported interaction path that the active QQ Bot reply target cannot honor
- **THEN** the Bridge falls back to a QQ Bot-supported text or follow-up delivery before sending the message
- **AND** the fallback metadata explains that the richer QQ Bot update plan was not usable
