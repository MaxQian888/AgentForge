...
- **THEN** the response includes a readable summary and the created subtasks
...
- **THEN** the system MUST NOT create any subtasks
...
- **THEN** every created task MUST belong to the same project as the parent task
- **THEN** every created task MUST start in an inbox-equivalent status that can be triaged later
...
- **THEN** the stored child tasks MUST NOT contain values that violate the task persistence contract
...
- **THEN** the system MUST NOT append another batch of subtasks
...
- **THEN** the system MUST leave the parent task without a partially created decomposition batch
...
- **THEN** the user MUST receive a result message that includes the decomposition summary and created subtasks
...
- **THEN** the message MUST explain that no new subtasks were created