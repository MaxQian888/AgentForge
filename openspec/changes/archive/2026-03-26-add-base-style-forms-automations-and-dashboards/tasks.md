...
- [x] 6.1 Implement `automation_engine_service.go` with EvaluateRules: on event, find matching rules, evaluate conditions, execute actions sequentially, log results
- [x] 6.2 Implement action executors: UpdateFieldAction, AssignUserAction, SendNotificationAction, MoveToColumnAction, CreateSubtaskAction, SendIMAction, InvokePluginAction
- [x] 6.3 Implement cascade prevention: check triggered_by_automation flag and skip evaluation
- [x] 6.4 Integrate automation evaluation into task_service (status change, assignee change, field change), review_service (review complete), and budget_governance_service (threshold reached)
- [x] 6.5 Implement due-date-approaching scheduler: periodic check (every 15min) for tasks with due dates within configured threshold
- [x] 6.6 Write service unit tests for engine, each action executor, and cascade prevention
...
- [x] 10.1 Install `recharts` for chart rendering
- [x] 10.2 Create `lib/stores/custom-field-store.ts` — field defs, field values per task
- [x] 10.3 Create `lib/stores/saved-view-store.ts` — views, current view, view switching
- [x] 10.4 Create `lib/stores/form-store.ts` — form defs, submissions
- [x] 10.5 Create `lib/stores/automation-store.ts` — rules, logs
- [x] 10.6 Create `lib/stores/dashboard-store.ts` — dashboard configs, widget data
- [x] 10.7 Create `lib/stores/milestone-store.ts` — milestones, roadmap data
- [x] 10.8 Write store unit tests

## 11. Frontend — Custom Fields UI

- [x] 11.1 Create `components/fields/field-definition-editor.tsx` — admin UI for creating/editing field defs in project settings
- [x] 11.2 Create `components/fields/field-value-cell.tsx` — inline editable cell for each field type (text, number, select, date, user, url, checkbox)
- [x] 11.3 Create `components/fields/field-filter-control.tsx` — filter input per field type
- [x] 11.4 Integrate custom field columns into task table/list views (`components/tasks/project-task-workspace.tsx`)
- [x] 11.5 Integrate custom field properties into task detail panel

## 12. Frontend — Saved Views UI

- [x] 12.1 Create `components/views/view-switcher.tsx` — dropdown in workspace header listing saved views
- [x] 12.2 Create `components/views/save-view-dialog.tsx` — dialog for naming and configuring view visibility
- [x] 12.3 Create `components/views/view-share-dialog.tsx` — dialog for sharing view with roles/members
- [x] 12.4 Integrate view switcher into project-task-workspace header

## 13. Frontend — Forms UI

- [x] 13.1 Create `components/forms/form-builder.tsx` — admin UI for creating/editing form definitions with field mapping
- [x] 13.2 Create `components/forms/form-renderer.tsx` — renders a form for submission (used in both dashboard and public URL)
- [x] 13.3 Create `app/(dashboard)/forms/[slug]/page.tsx` — public form submission page
- [x] 13.4 Add forms management section to project settings

## 14. Frontend — Automation UI

- [x] 14.1 Create `components/automations/rule-editor.tsx` — form for creating/editing rules: event picker, condition builder, action configurator
- [x] 14.2 Create `components/automations/rule-list.tsx` — list of rules with enable/disable toggles
- [x] 14.3 Create `components/automations/automation-log-viewer.tsx` — searchable/filterable activity log
- [x] 14.4 Add automations tab to project settings

## 15. Frontend — Dashboard UI

- [x] 15.1 Create `app/(dashboard)/project/dashboard/page.tsx` — dashboard route
- [x] 15.2 Create `components/dashboard/dashboard-grid.tsx` — grid layout with drag-and-drop widget positioning
- [x] 15.3 Create `components/dashboard/widget-wrapper.tsx` — common widget frame with title, config button, refresh
- [x] 15.4 Create widget components: ThroughputChart, BurndownChart, BlockerCount, BudgetConsumption, AgentCost, ReviewBacklog, TaskAging, SLACompliance
- [x] 15.5 Create `components/dashboard/add-widget-dialog.tsx` — widget type selector with preview
- [x] 15.6 Add "Dashboard" entry to sidebar navigation

## 16. Frontend — Milestone & Roadmap UI

- [x] 16.1 Create `components/milestones/milestone-editor.tsx` — create/edit milestone dialog
- [x] 16.2 Create `components/milestones/roadmap-view.tsx` — timeline view with milestone lanes, sprint blocks, and release markers
- [x] 16.3 Add milestone selector to task detail panel and sprint settings
- [x] 16.4 Add "Roadmap" view option to project task workspace view switcher

## 17. Integration — Automation Event Sources

- [x] 17.1 Wire task status/assignee/field changes to emit automation events
- [x] 17.2 Wire review completion to emit review.completed automation event
- [x] 17.3 Wire budget threshold crossing to emit budget.threshold_reached automation event
- [x] 17.4 Wire automation send_notification action to desktop notification delivery
- [x] 17.5 Wire automation send_im_message action to IM bridge
- [x] 17.6 Write integration tests for end-to-end automation flows
