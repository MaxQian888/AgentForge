...
- review operator workspace 已支持在 UI 中筛选 `pending_human`、触发手动 deep review、以及直接 approve / request changes。
- 官方 built-in ReviewPlugin 目前由 `plugins/builtin-bundle.yaml` 管理，`architecture-check` 与 `performance-check` 参与 Layer 2 聚合时与自定义 ReviewPlugin 使用同一条 provenance / finding contract。与它们并行配套的官方 control/start workflow 资产则是 `review-control` 与 `review-escalation-flow`，用于把 review trigger、detail inspect 与 approval pause 统一进同一套 starter catalog。
...
Layer 2 使用 Claude Agent SDK（TypeScript）构建自定义 Review Agent，通过 Agent SDK Bridge 的 canonical HTTP + WebSocket contract 与 Go 后端通信。

```
Go Review Service
  │
  │  HTTP `/bridge/review` + WS 事件流
  ▼
