...
本文档描述 AgentForge 编排层的完整蓝图，当前仓库已具备可工作的实现主线。阅读后续章节时，请优先以以下 live contract 理解系统实际运行方式：

- TS Bridge 的 canonical HTTP surface 以 `/bridge/*` 为准，已落地 `execute`、`decompose`、`classify-intent`、`generate`、`review`、`status`、`runtimes`、`cancel`、`pause`、`resume`、`health`、`active`、`pool`、`shell`、`thinking`、`mcp-status`，兼容 alias 仅用于历史调用迁移。
- Go 侧公开的 AI/Bridge 入口包含 `GET /api/v1/bridge/health`、`GET /api/v1/bridge/runtimes`、`POST /api/v1/ai/generate`、`POST /api/v1/ai/classify-intent`，而非早期 gRPC 服务发现模型。
- 编码运行时支持 7 个 runtime：`claude_code`、`codex`、`opencode`、`cursor`、`gemini`、`qoder`、`iflow`。Go 负责将 `runtime/provider/model` 元组解析后传给 Bridge；各 runtime 通过 catalog 暴露 `interaction_capabilities`、`launch_contract` 与 `lifecycle` 元数据，上游可按能力矩阵做准入判断。
- Codex 连续性通过 `codex exec --json` / `codex exec resume <thread-id>` 保留 `thread_id`；OpenCode 通过 `OPENCODE_SERVER_URL` 指向的 `/session` / `/prompt_async` / `/abort` 维持上游 `session_id`；不支持 pause/resume 的 CLI-backed runtime（cursor、gemini、qoder、iflow）会返回 `continuity_not_supported` 而非静默重放。
- iFlow 已进入 sunset 窗口，catalog 中标记为 `sunsetting` 并附带迁移指引至 Qoder。
- 实时事件通过 TS Bridge 主动连回 Go 的 WebSocket hub，再由 Go 广播到前端与其他消费者；当前主链路不是 Redis Streams 驱动的多跳架构。
- Team 生命周期上下文已显式进入运行时请求：planner / coder / reviewer 携带 `team_id`、`team_role` 与统一 runtime tuple。
- 官方 built-in starter catalog 包含 `task-control` / `review-control` / `workflow-control` 三个 MCP control tools，以及 `standard-dev-flow` / `task-delivery-flow` / `review-escalation-flow` 三个 workflow starters。
- Bridge ACP 客户端集成已完成（T0–T8），支持 OpenCode 权限请求映射、provider OAuth 路由与暂停会话控制连续性。
- 触发器引擎（`src-go/internal/trigger/`）已具备 CRUD、幂等性、规则路由、调度 tick、dry-run 与流集成测试能力；自动化规则（`src-go/internal/automation/`）由触发器引擎驱动执行。
- Employee（agent 身份）系统已落地：persistent agent 实体、role binding、生命周期、run history 与 trigger 配置。
...
> - [技术挑战全景分析](./technical-challenges.md)
> - [审查流水线设计](./review-pipeline-design.md)
> - [数据架构与实时系统设计](./data-realtime-design.md)
> - [cc-connect 复用指南](./cc-connect-reuse-guide.md)
