...
- **DB schema:** breaking changes allowed. Pre-flight migration changes (all covered by one new migration bundle):
  ...
  - New table `workflow_secret_materialisations` (§5.3 bullet 3) with FK to `workflow_executions(id)` ON DELETE CASCADE. Hash-only, 24 h TTL.
...
- Bridge-side log forwarding: the Bridge's review pipeline forwards LLM transcripts **asynchronously** — logs can arrive minutes after the originating `llm_agent` handler returns, so an in-call secret set is useless on the reingest path. Scope constraint: this spec is Go-orchestrator only; Bridge code changes are out of scope. Approach:
  1. On materialisation, the resolver stores a **hash-only** index per `(execution_id, node_id)`: for each `SecretRef` it materialises, it writes `sha256(cleartext)` into a new `workflow_secret_materialisations` table with columns `(execution_id UUID, node_id TEXT, secret_key TEXT, value_hash BYTEA, created_at TIMESTAMPTZ, expires_at TIMESTAMPTZ)`. The table holds hashes only — never the cleartext — and has `ON DELETE CASCADE` against `workflow_executions(id)`.
  2. `expires_at` defaults to `created_at + 24h`; the wait-event janitor (§7.3) also deletes expired rows in the same pass.
  3. The Bridge-log reingest path queries `value_hash` rows for the matching `(execution_id, node_id)`, hashes each substring of incoming log lines (rolling, bounded window), and replaces matches with `<secret:<secret_key>>` before persisting.
  4. Bridge-side native `SecretRef` traversal (preferred long-term) is captured in §11 Backlog; once shipped, this hash-index table is retired.
...
- Janitor job `workflow_wait_event_janitor` follows the same pattern as the existing built-in jobs (`task-progress-detector`, `automation-due-date-detector`, `bridge-health-reconcile`): the pre-flight migration seeds a `scheduled_jobs` row with `job_key='workflow_wait_event_janitor'` and a daily 03:00 cron; `cmd/server/main.go` registers the handler at startup via `schedulerSvc.RegisterHandler("workflow_wait_event_janitor", NewWaitEventJanitor(...))`. The handler runs two DELETEs in one transaction to cover orphan rows correctly (SQL's NULL propagation makes a single `max(...)` expression leak rows where `executions.completed_at IS NULL`):
  ```sql
  -- completed runs: keep 14 days of debugging history
  DELETE FROM wait_event_deliveries d
   USING workflow_executions e
   WHERE d.execution_id = e.id
     AND e.completed_at IS NOT NULL
     AND e.completed_at < now() - INTERVAL '14 days';
  -- orphan / long-running rows: absolute 30-day cap keyed on created_at
  DELETE FROM wait_event_deliveries
   WHERE created_at < now() - INTERVAL '30 days';
  ```
  The same handler also sweeps `workflow_secret_materialisations` for rows where `expires_at < now()` (§5.3 bullet 3).
...
- v1 definitions are view-only (cannot be instantiated). The existing instantiation surfaces in `WorkflowTemplateService` (`CloneTemplate`, `CreateFromTemplate`) are the entry points; P4 adds the status gate *inside those existing methods* (one shared helper `assertInstantiable(def) error` invoked at the top of each), rather than introducing a parallel `Instantiate` method. P4 enumerates all callsites to confirm no bypass:
  - `workflow_template_service.go` — `CloneTemplate`, `CreateFromTemplate`
  - `workflow_template_marketplace.go` — any install path that materialises a definition
  - `workflow_handler.go` — any handler endpoint that clones a template by ID
  Each callsite is migrated; §13 DoD gains an assertion that `grep -n 'CloneTemplate\|CreateFromTemplate' src-go/` yields zero uses that bypass `assertInstantiable`.
...
1. **Pre-flight migration.** One migration bundle landing before P1 that applies all four schema changes enumerated in §4:
   - `workflow_executions.idempotency_key` + unique index
   - `workflow_definitions.node_matrix_version` + widened `workflow_definitions_status_check` CHECK constraint (to accept `'deprecated'`)
   - `workflow_node_executions.effect_payload` JSONB column
   - `wait_event_deliveries` table + FK to `workflow_executions(id)` ON DELETE CASCADE + seeded `scheduled_jobs` row for the janitor (§7.3)
   - `workflow_secret_materialisations` table (§5.3 bullet 3)
