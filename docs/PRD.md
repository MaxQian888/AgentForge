...
flowchart LR
    subgraph CMD["命令通道 Go主动调TS"]
        ...
        Go --> E1["POST /execute 执行Agent任务"]
        Go --> E2["POST /tools/install 安装MCP Server"]
        Go --> E3["POST /abort 终止Agent"]
        Go --> E4["GET /health 健康检查"]
    end
...
sequenceDiagram
    ...
    Go->>TS: POST /execute (HTTP返回taskId)
...
Bridge 对轻量 AI 调用（任务分解、意图识别）暴露专用 HTTP 接口：

- `POST /api/analyze` — 任务分解
- `POST /api/classify-intent` — 意图识别
...
sequenceDiagram
    ...
    Go->>TS: POST /api/execute {taskId, prompt, worktree, maxTurns, budgetUsd, roleConfig}
    TS-->>Go: {sessionId}
    Go->>TS: POST /api/pause {sessionId}
    TS-->>Go: {snapshot}
    Go->>TS: POST /api/resume {sessionId, snapshot}
    TS-->>Go: {ok}
    Go->>TS: POST /api/cancel {sessionId}
    TS-->>Go: {ok}
    Go->>TS: GET /api/status/:sessionId
    TS-->>Go: {status, turn, tokens}
    Go->>TS: POST /api/analyze {requirement, context}
    TS-->>Go: {subtasks[]}
    Go->>TS: POST /api/classify-intent {message, history}
...
- **超时配置**：Agent 执行可能持续数分钟，WebSocket 连接的 keepalive 需要适配长时间连接（`ping interval: 30s, pong timeout: 10s`）。HTTP 请求中的 `/agent/execute` 立即返回 `202 Accepted`，实际执行通过 WS 事件流异步推送。
...
func (am *AgentManager) executeAgent(ctx context.Context, inst *AgentInstance, task *Task, wt *WorktreeInfo) {
    ...
    resp, err := am.bridgeHTTP.Post(bridgeURL+"/agent/execute", "application/json", toJSON(reqBody))
...
- **统一 AI 出口**：任务分解的 LLM 调用统一经过 TS Bridge（POST `/ai/decompose`），Go 侧不直接调用 LLM API。Bridge 作为所有 AI 调用的统一出口。
...
IM 平台通过 Integration Plugin（go-plugin）架构接入。详细设计参见 **Section 3.5（IM 适配层）** 及 `docs/CC_CONNECT_REUSE_GUIDE.md`。
...
> 来源：`docs/PLUGIN_RESEARCH_PLATFORMS.md`、`docs/PLUGIN_RESEARCH_ROLES.md`、`docs/PLUGIN_RESEARCH_TECH.md`
...
> 来源：`docs/AGENT_ORCHESTRATION.md` Section 9
...
| **Integration Plugin** | 集成插件，通过 go-plugin (gRPC 子进程) 对接外部系统（IM/CI/CD/云服务） |
| **Review Plugin** | 审查插件，通过 MCP + go-plugin 添加自定义代码审查规则 |
...
> - 插件系统详细设计: `docs/PLUGIN_SYSTEM_DESIGN.md`
> - Agent 编排详细设计: `docs/AGENT_ORCHESTRATION.md`
> - 数据架构详细设计: `docs/DATA_AND_REALTIME_DESIGN.md`
> - 审查流水线详细设计: `docs/REVIEW_PIPELINE_DESIGN.md`
