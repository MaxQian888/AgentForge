...
路径 2（所有后端 AI）: Go → gRPC → Bridge → Claude Agent SDK / Vercel AI SDK → LLM
...
```
  ...
  │                                                                    │
  │  ┌──────────────┐ ┌────────────────┐ ┌─────────────────────────┐  │
  │  │ gRPC Server   │ │ Agent Runtime  │ │ Session Manager          │  │
  │  │               │ │ Pool           │ │                          │  │
  │  │ · Execute()   │ │                │ │ · 会话快照               │  │
  │  │ · GetStatus() │ │ · query() ×N   │ │ · 暂停/恢复             │  │
  │  │ · Cancel()    │ │ · 工具沙箱     │ │ · 崩溃恢复              │  │
  │  │ · Health()    │ │ · Hook 链      │ │                          │  │
  │  └──────────────┘ └────────────────┘ └─────────────────────────┘  │
  │                           │                                        │
...
Task Service → Orchestration Core → Agent Pool Manager
    ...
    └── 3. Agent Pool Manager: 选择/创建 Agent 实例
        ...
        Bridge 调用 query({
            ...
            │
            ▼
        Claude API ← 流式 token →  Bridge ← gRPC Stream → Go Backend
                                                                │
                                                    ┌───────────┼───────────┐
                                                    ▼           ▼           ▼
...
Agent SDK Bridge 是一个独立的 TypeScript/Node.js 微服务，其核心职责是：

1. 封装 `@anthropic-ai/claude-agent-sdk` 的 `query()` 调用
2. 通过 gRPC 双向流暴露 Agent 执行能力给 Go Orchestrator
...
```
Claude API ──token──→ Agent SDK ──message──→ Bridge ──AgentEvent──→ gRPC Stream
   (HTTP SSE)        (async iter)          (proto encode)         (HTTP/2 frames)
                                                                        │
                                                                  Go Orchestrator
                                                                        │
                                                              ┌─────────┼─────────┐
                                                    ...
                                                    ┌─────────┼─────────┐
                                                    ▼                   ▼
                                               前端 Dashboard      IM Bridge
```

Go 侧使用 goroutine 扇出模式消费 gRPC Stream：
