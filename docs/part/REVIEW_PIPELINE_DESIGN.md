...
- 官方 built-in ReviewPlugin 目前由 `plugins/builtin-bundle.yaml` 管理，`architecture-check` 与 `performance-check` 参与 Layer 2 聚合时与自定义 ReviewPlugin 使用同一条 provenance / finding contract。
