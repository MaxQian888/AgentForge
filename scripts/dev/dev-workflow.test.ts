...
describe("dev-workflow command detection", () => {
  ...
  test("uses the Windows cmd wrapper when reading the pnpm version", () => {
    setProcessPlatform("win32");

    const spawnSync = jest.fn((command, args) => {
      if (command === "cmd.exe" && args[3] === "pnpm --version") {
        return {
          status: 0,
          stdout: "10.30.3\r\n",
          stderr: "",
    ...
    const { checkPrerequisiteVersions } = require("./dev-workflow.js");
