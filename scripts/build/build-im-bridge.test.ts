...
describe("desktop command contract", () => {
  test("exposes IM bridge and shared desktop prepare commands at the root", () => {
    ...
    expect(packageJson.scripts).toMatchObject({
      "build:im-bridge": "node scripts/build/build-im-bridge.js",
      "build:im-bridge:dev": "node scripts/build/build-im-bridge.js --current-only",
      "desktop:dev:prepare":
  ...
  test("uses the canonical build script family paths in deployment docs", () => {
    const desktopBuildDoc = fs.readFileSync(
      path.join(process.cwd(), "docs", "deployment", "desktop-build.md"),
      "utf8",
    );

    expect(desktopBuildDoc).toContain("scripts/build/build-im-bridge.js");
  });
});
