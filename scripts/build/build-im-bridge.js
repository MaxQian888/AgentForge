...
function getDirectories() {
  ...
  return {
    repoRoot,
    imBridgeDir: path.join(repoRoot, "src-im-bridge"),
    binariesDir: path.join(repoRoot, "src-tauri", "binaries"),
  };
}

function getOutputFilename(target) {
  return `im-bridge-${target.triple}${target.extension ?? (target.goos === "windows" ? ".exe" : "")}`;
}

function buildTarget(target, { imBridgeDir, binariesDir }) {
...
function main(argv = process.argv.slice(2)) {
  ...
  if (!currentOnly) {
    console.log("Cross-compiling IM bridge for all supported platforms...");
  }
  ...
  console.log("");
  console.log(
    `IM bridge build complete. Binaries in: ${directories.binariesDir}`,
  );
}
...
if (require.main === module) {
  try {
    ...
    console.error(
      error instanceof Error ? error.message : "Unknown IM bridge build error",
    );
    process.exit(1);
  }
}
