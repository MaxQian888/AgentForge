...
function scaffoldWorkflowPlugin({ rootDir, name, displayName, pluginId, version, modulePath }) {
  ...
  const buildScript = toImportPath(path.join(pluginDir, "package.json"), path.join(rootDir, "scripts", "plugin", "build-go-wasm-plugin.js"));
  const verifyScript = toImportPath(path.join(pluginDir, "package.json"), path.join(rootDir, "scripts", "plugin", "verify-plugin-dev-workflow.js"));
