...
- `Go Orchestrator` 是 `TS Bridge` 与 `IM Bridge` 之间唯一的后端中枢
- `IM Bridge` 通过 Go 侧 `/api/v1/*` 能力面访问 workflow、runtime diagnostics 与 Bridge-backed AI
- `TS Bridge` 只通过 canonical `/bridge/*` 合同向 Go 暴露 runtime / AI surface，不直接发现或调用 IM Bridge 实例
- 所有面向 IM 的进度与终态回传，都要回到 Go control plane，再命中原始 `bridge_id` 与 reply target

```mermaid
flowchart LR
  IM[IM Bridge] --> GO[Go Orchestrator]
  Web[Next.js Dashboard] --> GO
  Desktop[Tauri 桌面壳] --> Web
  Desktop --> GO
...
- [`docs/api/`](./docs/api)：按模块整理的当前 REST API 参考
- [`docs/schema/`](./docs/schema)：PostgreSQL 与 Redis 结构/键模式说明
- [`docs/deployment/`](./docs/deployment)：部署、环境变量、桌面打包与 TLS 文档
- [`docs/security/`](./docs/security)：认证、Tauri 权限模型、安全最佳实践
- [`docs/adr/`](./docs/adr)：架构决策记录
- [`docs/guides/`](./docs/guides)：插件、前端组件、状态管理等实操指南
...
如果你想单独拉起后端调试栈、不带 Next.js 前端，可以直接运行：
...
- 启动或复用 `http://127.0.0.1:7779/im/health` 上的 IM Bridge
- 启动或复用 `http://127.0.0.1:3000` 上的 Next.js 前端
- 将托管 IM Bridge identity 落地到 `.codex/im-bridge-id`
- 将 repo-local 运行态元数据写入 `.codex/dev-all-state.json`
- 将托管服务日志写入 `.codex/runtime-logs/`

当前 `dev:backend` 的覆盖范围：
...
- 启动或复用 `http://127.0.0.1:7779/im/health` 上的 IM Bridge
- 将托管 IM Bridge identity 落地到 `.codex/im-bridge-id`
- 将 repo-local 运行态元数据写入 `.codex/dev-backend-state.json`
- 将托管服务日志写入 `.codex/runtime-logs/`

当前 `dev:backend:verify` 的覆盖范围：

- 启动或复用与 `pnpm dev:backend` 相同的后端托管栈
- 校验 Go 健康端点 `http://127.0.0.1:7777/health`
- 校验 TS Bridge 健康端点 `http://127.0.0.1:7778/bridge/health`
- 校验 IM Bridge 健康端点 `http://127.0.0.1:7779/im/health`
- 通过受管 IM Bridge 的 stub test port 注入一条零外部凭据的 IM 命令往返，并且只有捕获到非空回复才会报告成功
- 默认保留托管后的后端栈继续运行，便于立刻做后续调试

说明：

- `dev:all` 只覆盖本地 Web 模式开发，不替代 `pnpm tauri:dev`。
- 如果你先跑了 `pnpm dev:backend`，后续再执行 `pnpm dev:all` 时会复用健康的后端服务，只额外接管自己启动的前端。
- 如果必需端口已被非 AgentForge 进程占用，`dev:all` 会直接报告冲突，而不是重复启动一套服务。
- `dev:backend:verify` 是仓库支持的后端 smoke proof；它会按阶段报告 `startup`、健康检查、stub 命令注入和 reply capture 的断点，并默认保留托管服务继续运行，后续请配合 `pnpm dev:backend:status` / `logs` / `stop` 使用。
...
如果你想跑完整的本地联调链路，优先使用 `pnpm dev:all`。如果你想单独拉起后端调试栈，优先使用 `pnpm dev:backend`。如果你想执行 Go + TS Bridge + IM Bridge 的仓库支持 smoke 验证，优先使用 `pnpm dev:backend:verify`。下面的命令仍然适合只调试 Go 服务本身。
...
- `GET /bridge/runtimes` 现在会在保留兼容字段 `supported_features` 的同时，额外发布分组后的 `interaction_capabilities`，按 `inputs`、`lifecycle`、`approval`、`mcp`、`diagnostics` 描述各 runtime 的真实交互能力，方便上游区分 supported / degraded / unsupported。
- Bridge 的高级 runtime control 现在新增 `POST /bridge/shell`（OpenCode session shell 命令）、`POST /bridge/thinking`（Claude thinking budget 调整）和 `GET /bridge/mcp-status/:task_id`（Claude Query 的 MCP 状态查询）。如果某个 runtime 不 truthfully 支持对应 control，Bridge 会返回带 `{ error, operation, runtime, support_state, reason_code }` 的结构化错误，而不是笼统的 500。
...
如果你想在前端已经运行的前提下，只单独调试 Rust/Tauri 运行时，可以使用 standalone 桌面调试流：
...
- Tauri 现在会同时监督三个必需 sidecar：`http://127.0.0.1:7777` 上的 Go Orchestrator、`http://127.0.0.1:7778` 上的 TS Bridge，以及 `http://127.0.0.1:7779` 上的 IM Bridge。
- 只有三个 sidecar 都通过健康检查后，桌面运行态才会被标记为 `ready`。异常退出会先做有界重启，超过阈值后才进入 `degraded`。
- `pnpm desktop:standalone:check` 与 `pnpm desktop:standalone:dev` 复用同一套 current-host sidecar prepare contract，但它们把 `http://localhost:3000` 前端可用性视为外部前置条件，不会替你隐式启动 `pnpm dev`。
- 维护中的 VS Code `Tauri Standalone Rust Debug` 入口也遵循同一规则：只准备当前主机 sidecar，不接管前端启动。
...
| `pnpm desktop:dev:prepare`      | 为桌面开发准备当前主机的 backend + TS Bridge + IM Bridge sidecar             |
| `pnpm desktop:standalone:check` | 检查 standalone Rust 桌面调试前置条件，但不启动前端                         |
| `pnpm desktop:standalone:dev`   | 在前端已运行时启动 standalone Rust 桌面调试流程                             |
| `pnpm desktop:build:prepare`    | 为桌面打包准备 backend + TS Bridge + IM Bridge sidecar，并构建前端 bundle    |
| `pnpm dev:all`                  | 启动或复用完整本地 Web 开发栈：compose infra + Go + TS Bridge + IM Bridge + frontend |
...
| `pnpm dev:backend`              | 启动或复用后端调试栈：compose infra + Go + TS Bridge + IM Bridge            |
| `pnpm dev:backend:status`       | 输出后端调试栈的来源、健康状态、端口与已知日志路径                         |
| `pnpm dev:backend:logs`         | 查看当前被 `dev:backend` 跟踪的 repo-local 日志文件路径                    |
| `pnpm dev:backend:stop`         | 仅停止 `dev:backend` 托管的服务，并保留复用的外部监听器                    |
| `pnpm dev:backend:verify`       | 运行仓库支持的后端 runtime smoke 工作流：启动/复用后端栈、校验健康 hop、注入一条 IM stub 命令往返，并保留托管服务继续运行 |
...
| `pnpm tauri:dev`                | 通过共享 prepare hook 启动完整 Tauri 开发模式                                |
...
- `pnpm desktop:standalone:check` 会检查 standalone Rust 桌面调试所需的前端入口、sidecar 二进制与端口冲突，但不会启动前端。
- `pnpm desktop:standalone:dev` 会在前端已就绪时单独启动 Rust/Tauri 运行时，便于聚焦 `src-tauri` 调试。
...
- 维护中的 VS Code `Tauri Standalone Rust Debug` 入口只会准备当前主机 sidecar，前端需要提前单独启动。
...
| `pnpm desktop:standalone:check` | 检查 standalone Rust 桌面调试前置条件                           |
| `pnpm desktop:standalone:dev`   | 在前端已运行时启动 standalone Rust 桌面调试流程                 |
