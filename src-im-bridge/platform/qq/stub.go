...
func (s *Stub) Name() string { return "qq-stub" }
...
func (s *Stub) ReplyContextFromTarget(target *core.ReplyTarget) any {
	...
	return replyContext{
		...
		IsGroup:   strings.TrimSpace(target.UserID) != "" && firstNonEmpty(target.ChatID, target.ChannelID, target.ConversationID) != strings.TrimSpace(target.UserID),
	}
}
...
func (s *Stub) Start(handler core.MessageHandler) error {
	...
	log.WithFields(log.Fields{"component": "qq-stub", "port": s.port}).Info("Test server starting")
	go func() {
		if err := s.server.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.WithField("component", "qq-stub").WithError(err).Error("Server error")
		}
	}()
	return nil
}
...
func (s *Stub) Reply(ctx context.Context, rawReplyCtx any, content string) error {
	...
	chatID := firstNonEmpty(reply.ChatID, reply.UserID)
	if !reply.IsGroup && reply.UserID != "" {
		chatID = "user:" + reply.UserID
	}
...
func (s *Stub) Send(ctx context.Context, chatID string, content string) error {
	...
	s.mu.Unlock()
	log.WithFields(log.Fields{"component": "qq-stub", "chat_id": chatID}).Info("Send: " + content)
	return nil
}
...
func (s *Stub) handleTestMessage(w http.ResponseWriter, r *http.Request) {
	...
	reply := &core.ReplyTarget{
		...
		Metadata: map[string]string{
			"message_type": map[bool]string{true: "group", false: "private"}[req.IsGroup],
		},
	}
	if !req.IsGroup {
