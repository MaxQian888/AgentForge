...
func (s *Stub) Name() string { return "wechat-stub" }

func (s *Stub) Metadata() core.PlatformMetadata {
	return core.NormalizeMetadata(baseMetadata(), "wechat")
}
...
func (s *Stub) ReplyContextFromTarget(target *core.ReplyTarget) any {
	...
	return replyContext{
		OpenID: firstNonEmpty(target.UserID),
...
func (s *Stub) Start(handler core.MessageHandler) error {
	...
	log.WithFields(log.Fields{"component": "wechat-stub", "port": s.port}).Info("Test server starting")
	go func() {
		if err := s.server.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.WithField("component", "wechat-stub").WithError(err).Error("Server error")
		}
	}()
	return nil
}
...
func (s *Stub) Reply(ctx context.Context, replyCtx any, content string) error {
	...
	target := firstNonEmpty(reply.ChatID, reply.OpenID)
...
func (s *Stub) recordReply(chatID, content, nativeSurface, format string) error {
	...
	s.mu.Unlock()
	log.WithFields(log.Fields{"component": "wechat-stub", "chat_id": chatID}).Info("Send: " + content)
	return nil
}
...
var _ core.NativeMessageSender = (*Stub)(nil)
...
func (s *Stub) handleTestMessage(w http.ResponseWriter, r *http.Request) {
	...
	if req.UserID == "" {
		req.UserID = "wechat-user"
	}
	if req.UserName == "" {
		req.UserName = "WeChat User"
	}
	if req.ChatID == "" {
		req.ChatID = "wechat-chat"
	}
	if req.ResponseURL == "" {
		req.ResponseURL = "https://api.weixin.qq.com/response"
	}

	msg := &core.Message{
		Platform:   "wechat",
		SessionKey: fmt.Sprintf("wechat:%s:%s", req.ChatID, req.UserID),
		UserID:     req.UserID,
...
func chatIDFromReplyContext(replyCtx any) string {
	...
	case replyContext:
		return firstNonEmpty(value.ChatID, value.OpenID)
	case *replyContext:
		if value != nil {
			return firstNonEmpty(value.ChatID, value.OpenID)
		}
