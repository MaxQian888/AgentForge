...
func (s *Stub) Start(handler core.MessageHandler) error {
	...
	log.WithFields(log.Fields{"component": "discord-stub", "port": s.port}).Info("Test server starting")
	go func() {
		if err := s.server.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.WithField("component", "discord-stub").WithError(err).Error("Server error")
		}
	}()
	return nil
}
...
func (s *Stub) SendCard(ctx context.Context, chatID string, card *core.Card) error {
	...
	return s.recordReply(chatID, fallback, "discord_card", "")
}
...
func (s *Stub) ReplyCard(ctx context.Context, replyCtx any, card *core.Card) error {
	...
	return s.recordReply(chatIDFromReplyContext(replyCtx), fallback, "discord_card", "")
}
...
func (s *Stub) handleTestMessage(w http.ResponseWriter, r *http.Request) {
	...
	if req.UserID == "" {
		req.UserID = "discord-user"
	}
	if req.UserName == "" {
		req.UserName = "Discord User"
	}
	if req.ChatID == "" {
		req.ChatID = "discord-channel"
	}
	...
	msg := &core.Message{
		...
		ReplyTarget: &core.ReplyTarget{
			...
			InteractionToken: "stub-interaction-token",
			UseReply:         true,
		},
	}
	msg.ReplyCtx = msg
