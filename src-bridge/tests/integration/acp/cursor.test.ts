/**
 * ACP integration test — cursor adapter.
 *
 * Gated by SKIP_ACP_INTEGRATION !== "0" (default: skipped).
 * No API key required — cursor uses its own auth; user must be logged in via cursor-agent.
 *
 * Requires `cursor-agent` CLI to be installed.
 *
 * Usage:
 *   SKIP_ACP_INTEGRATION=0 bun test tests/integration/acp/cursor.test.ts
 */
...
// cursor has no required env — user must be logged in via cursor-agent.
...
d("ACP integration — cursor", () => {
  test(
    "smoke — prompt('echo hello') → output event + stopReason",
    async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const events: any[] = [];
      const cwd = mkdtempSync(path.join(tmpdir(), "acp-int-cursor-smoke-"));
      const { session, pool } = await makeSession({ cwd, taskId: "int-cursor-smoke", events });
  ...
  test(
    ...
    async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const events: any[] = [];
      const cwd = mkdtempSync(path.join(tmpdir(), "acp-int-cursor-cancel-"));
      const { session, pool } = await makeSession({ cwd, taskId: "int-cursor-cancel", events });
  ...
  test(
    ...
    async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const events: any[] = [];
      const cwd = mkdtempSync(path.join(tmpdir(), "acp-int-cursor-fs-"));
      const inputFile = path.join(cwd, "input.txt");
      const outputFile = path.join(cwd, "output.txt");
      writeFileSync(inputFile, "hello from test");

      const { session, pool } = await makeSession({ cwd, taskId: "int-cursor-fs", events });
  ...
  test(
    ...
    async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const events: any[] = [];
      const cwd = mkdtempSync(path.join(tmpdir(), "acp-int-cursor-terminal-"));
      const { session, pool } = await makeSession({ cwd, taskId: "int-cursor-terminal", events });
  ...
  test(
    ...
    async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const events: any[] = [];
      const cwd = mkdtempSync(path.join(tmpdir(), "acp-int-cursor-perm-"));
