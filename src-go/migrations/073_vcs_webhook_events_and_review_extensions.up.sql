...
-- review_findings is not a persisted table in the current codebase (findings
-- are stored inline as JSON on the parent review row). The columns below were
-- reserved for a future VCS outbound table; guard with a DO block so the
-- migration is a no-op until the table actually exists.
DO $$
BEGIN
  IF EXISTS (
