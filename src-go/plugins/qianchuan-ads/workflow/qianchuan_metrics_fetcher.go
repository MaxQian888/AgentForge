...
// node type. It emits a single nodetypes.EffectFetchQianchuanMetrics effect; the
...
// {{$dataStore.<nodeID>.snapshot}} via nodetypes.ResolveTemplateVars.
type QianchuanMetricsFetcherHandler struct{}

func (QianchuanMetricsFetcherHandler) Execute(_ context.Context, req *nodetypes.NodeExecRequest) (*nodetypes.NodeExecResult, error) {
	var (
		bindingTpl string
		dims       []string
		binding    string
	...
	return &nodetypes.NodeExecResult{
		Effects: []nodetypes.Effect{{Kind: nodetypes.EffectFetchQianchuanMetrics, Payload: payload}},
	}, nil
}
...
func (QianchuanMetricsFetcherHandler) Capabilities() []nodetypes.EffectKind {
	return []nodetypes.EffectKind{nodetypes.EffectFetchQianchuanMetrics}
}
