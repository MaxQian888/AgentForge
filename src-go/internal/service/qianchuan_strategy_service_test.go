...
func TestQianchuanServiceCreate(t *testing.T) {
	...
	svc := service.NewQianchuanStrategyService(newFakeRepo())
	pid := uuid.New()
	row, err := svc.Create(ctx, service.QianchuanStrategyCreateInput{
		ProjectID:  &pid,
...
func TestQianchuanServiceUpdateRejectedOnPublished(t *testing.T) {
	...
	svc := service.NewQianchuanStrategyService(newFakeRepo())
	pid := uuid.New()
	row, _ := svc.Create(ctx, service.QianchuanStrategyCreateInput{ProjectID: &pid, YAMLSource: validYAML, CreatedBy: uuid.New()})
	if err := svc.Publish(ctx, row.ID); err != nil {
		t.Fatalf("Publish: %v", err)
	}
	_, err := svc.Update(ctx, row.ID, validYAML)
	if !errors.Is(err, service.ErrStrategyImmutable) {
...
func TestQianchuanServicePublishTwiceRejected(t *testing.T) {
	...
	if !errors.Is(err, service.ErrStrategyInvalidTransition) {
...
func TestQianchuanServiceDeleteOnlyDraft(t *testing.T) {
	...
	row2, _ := svc.Create(ctx, service.QianchuanStrategyCreateInput{ProjectID: &pid, YAMLSource: validYAML, CreatedBy: uuid.New()})
...
func TestQianchuanServiceEditAfterPublishRequiresNewCreate(t *testing.T) {
	...
	pid := uuid.New()
	user := uuid.New()
	row, _ := svc.Create(ctx, service.QianchuanStrategyCreateInput{ProjectID: &pid, YAMLSource: validYAML, CreatedBy: user})
	_ = svc.Publish(ctx, row.ID)

	// Subsequent Create with the same name returns a fresh draft at version=2.
	row2, err := svc.Create(ctx, service.QianchuanStrategyCreateInput{ProjectID: &pid, YAMLSource: validYAML, CreatedBy: user})
	if err != nil {
...
func TestQianchuanServiceWriteRejectsSystemRow(t *testing.T) {
	ctx := context.Background()
	repo := newFakeRepo()
	svc := service.NewQianchuanStrategyService(repo)
	...
	if err := svc.Publish(ctx, sysRow.ID); !errors.Is(err, service.ErrStrategySystemReadOnly) {
		t.Errorf("publish system: got %v", err)
	}
	if err := svc.Archive(ctx, sysRow.ID); !errors.Is(err, service.ErrStrategySystemReadOnly) {
		t.Errorf("archive system: got %v", err)
	}
	if err := svc.Delete(ctx, sysRow.ID); !errors.Is(err, service.ErrStrategySystemReadOnly) {
		t.Errorf("delete system: got %v", err)
	}
	if _, err := svc.Update(ctx, sysRow.ID, validYAML); !errors.Is(err, service.ErrStrategySystemReadOnly) {
...
func TestQianchuanServiceCreateBubblesParseErrors(t *testing.T) {
	...
	svc := service.NewQianchuanStrategyService(newFakeRepo())
	pid := uuid.New()
	_, err := svc.Create(ctx, service.QianchuanStrategyCreateInput{
