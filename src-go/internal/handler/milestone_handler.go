...
func (h *MilestoneHandler) List(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusInternalServerError, model.ErrorResponse{Message: "failed to list milestones"})
	}
	...
	for _, milestone := range milestones {
		...
		if metricsErr != nil {
			return c.JSON(http.StatusInternalServerError, model.ErrorResponse{Message: "failed to load milestone metrics"})
		}
		metricsCopy := metrics
...
func (h *MilestoneHandler) Create(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusBadRequest, model.ErrorResponse{Message: "invalid target date"})
	}
	...
	if err := h.service.CreateMilestone(c.Request().Context(), milestone); err != nil {
		return c.JSON(http.StatusInternalServerError, model.ErrorResponse{Message: "failed to create milestone"})
	}
...
func (h *MilestoneHandler) Update(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusBadRequest, model.ErrorResponse{Message: "invalid milestone ID"})
	}
	milestone, err := h.service.GetMilestone(c.Request().Context(), milestoneID)
	if err != nil || milestone == nil || milestone.ProjectID != projectID {
		return c.JSON(http.StatusNotFound, model.ErrorResponse{Message: "milestone not found"})
	}
	...
	if err := h.service.UpdateMilestone(c.Request().Context(), milestone); err != nil {
		return c.JSON(http.StatusInternalServerError, model.ErrorResponse{Message: "failed to update milestone"})
	}
...
func (h *MilestoneHandler) Delete(c echo.Context) error {
	...
	if err := h.service.DeleteMilestone(c.Request().Context(), milestoneID); err != nil {
		return c.JSON(http.StatusInternalServerError, model.ErrorResponse{Message: "failed to delete milestone"})
	}
