...
func (f *fakeQianchuanService) Create(_ context.Context, in service.QianchuanStrategyCreateInput) (*strategy.QianchuanStrategy, error) {
	if f.createErr != nil {
...
func (f *fakeQianchuanService) Update(_ context.Context, id uuid.UUID, yamlSource string) (*strategy.QianchuanStrategy, error) {
	...
	if row.Status != strategy.StatusDraft {
		return nil, service.ErrStrategyImmutable
	}
...
func (f *fakeQianchuanService) Publish(_ context.Context, id uuid.UUID) error {
	...
	if row.IsSystem() {
		return service.ErrStrategySystemReadOnly
	}
	if row.Status != strategy.StatusDraft {
		return service.ErrStrategyInvalidTransition
	}
...
func (f *fakeQianchuanService) Delete(_ context.Context, id uuid.UUID) error {
	...
	if row.Status != strategy.StatusDraft {
		return service.ErrStrategyImmutable
	}
...
func (f *fakeQianchuanService) TestRun(_ context.Context, id uuid.UUID, snapshot map[string]any) (*service.TestRunResult, error) {
	if _, ok := f.strategies[id]; !ok {
		return nil, repository.ErrNotFound
	}
	return &service.TestRunResult{
		FiredRules: []string{"hb"},
		Actions:    []service.TestRunResolvedAction{{Rule: "hb", Type: "notify_im", Params: snapshot}},
	}, nil
}
