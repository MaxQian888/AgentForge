...
type QianchuanStrategyService interface {
	Create(ctx context.Context, in service.QianchuanStrategyCreateInput) (*strategy.QianchuanStrategy, error)
	...
	TestRun(ctx context.Context, id uuid.UUID, snapshot map[string]any) (*service.TestRunResult, error)
}
...
func (h *QianchuanStrategiesHandler) Create(c echo.Context) error {
	...
	row, err := h.svc.Create(c.Request().Context(), service.QianchuanStrategyCreateInput{
		ProjectID:  &pid,
...
func mapServiceError(c echo.Context, err error) error {
	...
	if errors.Is(err, service.ErrStrategySystemReadOnly) {
		return c.JSON(http.StatusForbidden, errorBody{Error: err.Error()})
	}
	if errors.Is(err, service.ErrStrategyImmutable) || errors.Is(err, service.ErrStrategyInvalidTransition) {
