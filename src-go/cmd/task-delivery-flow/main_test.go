...
func TestDescribeExposesWorkflowMetadata(t *testing.T) {
	...
	if descriptor.ID != "task-delivery-flow" {
		t.Fatalf("descriptor.ID = %q, want %q", descriptor.ID, "task-delivery-flow")
	}
...
func TestInitHealthAndInvokeCoverHappyPathAndUnsupportedOperation(t *testing.T) {
	...
	if health.Data["workflow"] != "task-delivery-flow" {
		t.Fatalf("Health().Data[workflow] = %v, want task-delivery-flow", health.Data["workflow"])
	}
	...
	if result.Data["workflow"] != "task-delivery-flow" {
		t.Fatalf("Invoke(run_workflow).Data[workflow] = %v, want task-delivery-flow", result.Data["workflow"])
	}
...
func TestAgentforgeExportsUseSDKRuntime(t *testing.T) {
	...
	if envelope.Data["workflow"] != "task-delivery-flow" {
		t.Fatalf("envelope.Data[workflow] = %v, want task-delivery-flow", envelope.Data["workflow"])
	}
}
